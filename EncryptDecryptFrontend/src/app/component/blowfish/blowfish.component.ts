...
@Component({
  ...
  styleUrl: './blowfish.component.css'
})
...
copyButtonText = 'Copy';
  ...
  encrypt() {
    this.encryptionService.blowfishEncrypt(this.input).subscribe(res => {
  ...
  decrypt() {
    this.encryptionService.blowfishDecrypt(this.input).subscribe(res => {
      this.result = res.output;
    });
  }
  goBack(path: string) {
